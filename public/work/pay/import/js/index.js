...
var qc_work_pay_import = {
    ...
    pay: {
        ...
        savePay: function (form) {
            if (confirm('Tôi đồng ý thanh toán hóa dơn này')) {
...
$(document).ready(function () {
    $('.qc_work_pay_import_wrap').on('click', '.qc_import_pay_get', function () {
        qc_work_pay_import.pay.getPay($(this).data('href'));
    });

    $('body').on('click', '.qc_frm_pay_import_pay .qc_save', function () {
        qc_work_pay_import.pay.savePay($(this).parents('.qc_frm_pay_import_pay'));
    })
});
