...
class QcWorkAllocation extends Model
{
    ...
    // ket thuc cong viec
    public function confirmFinish($allocationId, $reportDate, $finishStatus, $finishReason = 0)
    {
        $hFunction = new \Hfunction();
        ...
        if (QcWorkAllocation::where('allocation_id', $allocationId)->update(['action' => 0])) {
            ...
            $modelWorkAllocationFinish->insert($reportDate, $finishStatus, $finishLevel, $finishReason, '', $allocationId);
        }
    }
    //========== ========= ========= RELATION ========== ========= ==========
    //---------- nhan vien ban giao -----------
