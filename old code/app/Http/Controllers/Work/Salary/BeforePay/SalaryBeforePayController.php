...
class SalaryBeforePayController extends Controller
{
    ...
    public function index($monthFilter = null, $yearFilter = null)
    {
        ...
        if (count($dataStaff) > 0) {
            if (count($dataStaff) > 0) {
                ...
                return view('work.salary.before-pay.index', compact('dataAccess', 'modelStaff','modelStaffWorkSalary', 'dataWork','monthFilter','yearFilter'));
            }
        } else {
    ...
    //đề suất ứng
    public function getBeforePayRequest($workId)
    {
        $modelStaff = new QcStaff();
        $modelWork = new QcWork();
