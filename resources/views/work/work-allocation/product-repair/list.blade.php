...
?>
@extends('work.work-allocation.product-repair.index')
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                    <h4 style="background-color: red; color: white; margin: 0; padding: 5px;">
                        Quản lý thi công phải chịu tracst nhiệm hoàn thành sửa chửa các sản phẩm này
                    </h4>
                </div>
            </div>
                ...
                <div class="qc_work_allocation_manage_list_content  row">
                    <div class="table-responsive">
                        <table class="table table-hover table-bordered">
                                ...
                                <td style="padding: 0px;">
                                    <div class="input-group">
                                        <input type="text" class="txtAllocationManageKeywordFilter form-control"
                                               ...
                                               placeholder="Tìm theo đơn hàng" value="{!! $orderFilterName !!}">
                            ...
                            @if($hFunction->checkCount($dataProductRepair))
                                <?php
                                $perPage = $dataProductRepair->perPage();
                                $currentPage = $dataProductRepair->currentPage();
                                ...
                                $sumUnPaidMoney = 0;
                                ?>
                                @foreach($dataProductRepair as $productRepair)
                                    <?php
                                    $repairId = $productRepair->repairId();
                                    $createdAt = $productRepair->createdAt();
                                    # san pham
                                    $dataProduct = $productRepair->product;

                                    # thong tin don hang
                                    $order = $dataProduct->order;
                                    ...
                                    <tr class="qc_ad3d_list_object @if($n_o%2 == 1) info @endif"
                                        ...
                                        <td class="text-center">
                                            {!! $hFunction->convertDateDMYFromDatetime($createdAt) !!}
                                        </td>
                                        <td class="text-left">
                                            @if(!$cancelStatus)
                                ...
                                <tr>
                                    <td class="text-center" colspan="11">
                                        {!! $hFunction->page($dataProductRepair) !!}
                                    </td>
                                </tr>
                            @else
                                <tr>
